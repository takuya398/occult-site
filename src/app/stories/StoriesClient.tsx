...
export default function StoriesClient() {
  ...
  const filteredStories = useMemo(() => {
    ...
    const matchesQuery = (story: (typeof stories)[number]) => {
    ...
    const matchesType = (story: (typeof stories)[number]) => {
      if (typeFilter === "all") return true;
      return story.type === typeFilter;
    };

    const matchesTags = (story: (typeof stories)[number]) => {
      if (selectedTags.length === 0) return true;
      return selectedTags.every((tag) => story.tags.includes(tag));
    };

    const matchesDanger = (story: (typeof stories)[number]) => {
      if (dangerFilter === "all") return true;
      const danger = story.danger ?? 0;
      if (dangerFilter === "5") return danger === 5;
      const threshold = Number(dangerFilter);
      return danger >= threshold;
    };

    const matchesCredibility = (story: (typeof stories)[number]) => {
      if (credibilityFilter === "all") return true;
      return story.credibility === credibilityFilter;
    };
    ...
    <div className="min-h-screen bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
      <div className="mx-auto w-full max-w-5xl px-6 py-12">
        ...
        <div className="mt-3 flex items-center justify-between text-sm text-zinc-600">
          <span>該当 {filteredStories.length} 件</span>
        </div>
        ...
        <section className="mt-8 grid gap-4 sm:grid-cols-2">
          {filteredStories.map((story) => (
            <CardLink
              key={story.slug}
              href={`/stories/${story.slug}${detailsSuffix}`}
              ariaLabel={`${story.title}の詳細へ`}
            >
              <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-500">
                {story.type && <TagChip variant="outline">{story.type}</TagChip>}
              </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-xs">
                {story.credibility && (
                  <Badge tone="emerald">信憑性 {story.credibility}</Badge>
                )}
                {story.danger && (
                  <Badge tone="rose">危険度 {story.danger}</Badge>
                )}
              </div>
            </CardLink>
          ))}
